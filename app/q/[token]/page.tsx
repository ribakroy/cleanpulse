import { findScreenByQrToken } from "@/lib/data/repositories/screens";
...
export const metadata = {
  title: "דיווח שירותים בנייד | CleanPulse",
};
...
export default async function QrPage({ params }: QrPageProps) {
  ...
  // 1. Resolve screen using qrToken
  const screen = await findScreenByQrToken(token);

  if (!screen || !screen.isActive) {
    return <PublicErrorState message="קישור ה-QR אינו תקין או שהמסך הושבת." />;
  }

  // 2. Resolve branch and restroom
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col justify-between p-4 sm:p-6">
      ...
      <footer className="py-4 text-center text-xs text-muted/50">
        CleanPulse &copy; {new Date().getFullYear()} · סרוק לדיווח מהיר
      </footer>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 text-center">
      <div className="surface-panel rounded-[var(--radius-xl)] max-w-md w-full p-8 space-y-6 shadow-panel">
        ...
        <p className="text-sm leading-7 text-muted">
          אנא פנה לצוות המקום לקבלת סיוע.
        </p>
      </div>
    </div>
  );
